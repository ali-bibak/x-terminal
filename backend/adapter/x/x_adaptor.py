...
if __name__ == "__main__":
    adaptor = XAdaptor(bearer_token=os.environ.get("X_BEARER_TOKEN"))
    tweets = adaptor.search_topic("XAI Hackathon", minutes=15, max_results=20)
    for tweet in tweets:
        print(f"[{tweet['created_at']}] @{tweet['username']}: {tweet['text']} (Likes: {tweet['likes']}, Retweets: {tweet['retweets']})")
        print("\n")
